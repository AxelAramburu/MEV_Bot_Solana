...
pub async fn load_all_important_tokens(
    provider: &Arc<Provider<Ws>>,
    block_number: U64,
    important_tokens: Vec<H160>,
) -> Result<HashMap<H160, Token>> {
    let cache_file = "cache/.cached-tokens-important.csv";
    let file_path = Path::new(cache_file);
    let file_exists = file_path.exists();
    let file = OpenOptions::new()
        .write(true)
    ...
    let pb = ProgressBar::new(important_tokens.len() as u64);
    pb.set_style(
        ProgressStyle::with_template(
            "[{elapsed_precise}] {bar:40.cyan/blue} {pos:>7}/{len:7} {msg}",
        )
