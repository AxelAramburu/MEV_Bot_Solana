...
impl DexVariant {
    ...
    pub fn str(&self) -> String {
        match self {
            DexVariant::UniswapV2 => "UniswapV2".to_owned(),
            DexVariant::UniswapV3 => "UniswapV3".to_owned(),
        }
    }
}
...
impl From<StringRecord> for Pool {
    fn from(record: StringRecord) -> Self {
        let version = match record.get(3).unwrap().parse().unwrap() {
            2 => DexVariant::UniswapV2,
            3 => DexVariant::UniswapV3,
            _ => DexVariant::UniswapV2,
        };
        let dex = match record.get(1).unwrap() {
        ...
        Self {
            ...
            dex,
            address: H160::from_str(record.get(2).unwrap()).unwrap(),
            version,
            token0: H160::from_str(record.get(4).unwrap()).unwrap(),
            token1: H160::from_str(record.get(5).unwrap()).unwrap(),
            fee: record.get(6).unwrap().parse().unwrap(),
            block_number: record.get(7).unwrap().parse().unwrap(),
            timestamp: record.get(8).unwrap().parse().unwrap(),
        }
    }
}

impl Pool {
    pub fn cache_row(&self) -> (i64, String, String, i32, String, String, u32, u64, u64) {
        (
            self.id,
            self.dex.str() as String,
            format!("{:?}", self.address),
...
pub async fn load_all_pools(
    ...
    let cache_file = "cache/.cached-pools-important.csv";
    ...
    let mut uni_v2_pool_cnt = 0;
    let mut uni_v3_pool_cnt = 0;
    ...
    if file_exists {
        ...
        for row in reader.records() {
            ...
            match pool.version {
                DexVariant::UniswapV2 => uni_v2_pool_cnt += 1,
                DexVariant::UniswapV3 => uni_v3_pool_cnt += 1,
            }
            pools.push(pool);
        }
    } else {
    ...
    info!("Uniswap V2 pools: {:?}", uni_v2_pool_cnt);
    info!("Uniswap V3 pools: {:?}", uni_v3_pool_cnt);
    ...
    // Uniswap V2
    let pair_created_event_uni_v2 = "PairCreated(address,address,address,uint256)";
    let abi_uni_v2 = parse_abi(&[&format!("event {}", pair_created_event_uni_v2)]).unwrap();
    let pair_created_signature_uni_v2 = abi_uni_v2.event("PairCreated").unwrap().signature();

    //Uniswap V3
    let pair_created_event_uni_v3 = "PoolCreated(address,address,uint24,int24,address)";
    let abi_uni_v3 = parse_abi(&[&format!("event {}", pair_created_event_uni_v3)]).unwrap();
    let pair_created_signature_uni_v3 = abi_uni_v3.event("PoolCreated").unwrap().signature();
    ...
    for range in block_range {
        ...
        requests.push(tokio::task::spawn(load_uniswap_v2_pools(
            ...
            range.0,
            range.1,
            pair_created_event_uni_v2,
            pair_created_signature_uni_v2,
        )));
        ...
        requests.push(tokio::task::spawn(load_uniswap_v3_pools(
            ...
            range.0,
            range.1,
            pair_created_event_uni_v3,
            pair_created_signature_uni_v3,
        )));
...
pub async fn load_uniswap_v3_pools(
    ...
    for log in logs {
        ...
        let token0 = H160::from(log.topics[1]);
        let token1 = H160::from(log.topics[2]);
        let last_4_bytes: [u8; 4] = log.topics[3].0[28..].try_into().expect("Invalid slice size");
        let fee = u32::from_be_bytes(last_4_bytes);
        if let Ok(input) =
            ethers::abi::decode(&[ParamType::Uint(256), ParamType::Address], &log.data)
        {   
            let pair = input[1].to_owned().into_address().unwrap();
            let pool_data = Pool {
                id: -1,
                dex: DexVariant::UniswapV3,
                address: pair,
